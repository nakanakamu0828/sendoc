                ...
                <div class="box">
                    <h1 class="has-text-centered">
                        <a href="{{ url('/dashboard') }}" class="has-text-black">
                            <span class="logo">{{ __('common.setting.account') }}</span>
                        </a>
                    </h1>
                    <form method="POST" action="{{ route('setting.account.update') }}" class="m-t-50">
                        @csrf
                        {{ method_field('PUT') }}
                        ...
                        <div class="field">
                            ...
                            <div class="control  has-icons-left">
                                <input id="input" type="email" class="input {{ $errors->has('email') ? ' is-danger' : '' }}" name="email" placeholder="{{ __('common.email') }}" value="{{ old('email', Auth::user()->email) }}">
